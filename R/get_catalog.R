...
#'
#'
get_catalog = function(dataset_uid = NULL){
  tryCatch({
    key = getKey()
    domain = getDomain()
    api_type = getApiType()
  ...
  if (is.null(dataset_uid)){
    link <- glue::glue("https://{domain}/api/explore/v2.1/catalog/datasets?apikey={key}&select=dataset_id%2Ctitle%2Cdataset_uid&limit={limit}&offset={offset}&timezone=UTC&include_links=false&include_app_metas=false")
  } else {
    link <- glue::glue("https://{domain}/api/explore/v2.1/catalog/datasets?apikey={key}&select=dataset_id%2Ctitle%2Cdataset_uid&where=dataset_uid%3D%22{dataset_uid}%22&limit=1&offset={offset}&timezone=UTC&include_links=false&include_app_metas=false")
    res = httr::GET(link)
